...
"
    ...
    for sec in subs:
        ...
        for jam in TIMES[waktu]:
            ...
            line.append(InlineKeyboardButton(f"{jam} {sym}", callback_data=f"toggle_{waktu}_{sec}_{jam}_{page}"))
        rows.append(line)
        # Activate section button full width
        rows.append([InlineKeyboardButton(f"🔄 Aktifkan {sec}", callback_data=f"activate_{waktu}_{sec}_{page}")])
    ...
    rows.append([InlineKeyboardButton("🔄 Reset Hari Ini", callback_data=f"reset_all_{waktu}_{page}")])
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("toggle_"):
        ...
        _, w, pg = parts
        # clear all skips and disable today's reminders
        user_skips.pop(cid, None)
        group_reminders.setdefault(cid, {}).update({w: False})
        await q.edit_message_text(
            f"🔁 Checklist dihapus dan pengingat {w} dipause untuk hari ini. Gunakan /aktifkan_{w} untuk mengaktifkan kembali.",
            parse_mode="Markdown"
        )
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    txt = (
        ...
        "Gunakan tombol atau perintah berikut."
    )
...
async def reset_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_skips.pop(cid, None)
    for w in TIMES: group_reminders.setdefault(cid, {})[w] = False
    await update.message.reply_text("🔁 Semua checklist direset dan pengingat dipause untuk hari ini.")

async def toggle_reminder_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    cid = update.effective_chat.id
    cmd = update.message.text.lstrip('/').split('@')[0]
    thread_id = update.message.message_thread_id
    if thread_id: group_threads[cid] = thread_id
    if cmd.endswith(('pagi','siang','malam')):
        ...
        await update.message.reply_text(f"✅ Pengingat {w} {'diaktifkan' if on else 'dinonaktifkan'}. Buat reset gunakan tombol di menu.")
